...
///
/// Acts at the parser/serializer edge per
/// `docs/requirements/fase-1/11-import-export.md`: "la conversión decimal →
/// centavos ocurre en el borde (parser/serializador), nunca dentro del
/// dominio" — it never becomes part of a domain entity's business rules. It
/// lives in `domain/utils/` (not `data/`), same as `text_normalizer.dart`,
/// because it is pure (no Drift/Flutter dependency) and is used both by the
/// real import conversion in `data/` and by the live mapping-preview sheets
/// in `presentation/`, which must not reach into `data/` directly.
...
abstract final class DecimalAmountParser {
  ...
  static ParsedDecimalAmount? parse(
    ...
    if (!_digitsOnly.hasMatch(integerPart) ||
        !_digitsOnly.hasMatch(fractionPart)) {
      return null;
    }
  ...
  static String format(int minorUnits, {required int currencyDecimals}) {
    ...
    final fractionPart =
        (minorUnits % scale).toString().padLeft(currencyDecimals, '0');
