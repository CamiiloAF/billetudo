...
- **Actualización 2026-07-19 (post-cierre):** 4 rondas de corrección de fidelidad visual contra `billetudo.pen` + 1 ronda de corrección del patrón de color por categoría + 1 ronda de unificación de componentes compartidos con Transacciones (categoría, campos Cuenta/Fecha, etiquetas, monto), todas fuera de la corrida original de `feature-dev`. Ver "Fidelidad visual — historial de correcciones" más abajo.
...
4. **Ronda 7 (unificación de componentes compartidos, alcance ampliado a Transacciones):** el usuario señaló que varios componentes del formulario de Pagos Programados divergían del de Transacciones sin razón — mismo dato, distinto widget. Se unificó en la fuente compartida en vez de mantener 2 copias:
   - **Categoría**: `CategoryPickerChip` (Transacciones, pill horizontal — nunca coincidió con `EIoVx`) reconstruido como el tile vertical real; `ScheduledPaymentCategoryTiles` (duplicado de Pagos Programados) borrado, ambas features usan `CategoryQuickPicker`. En Pencil, 12+ instancias sueltas de "Categoria Chips" (incluidas las de Pagos Programados) consolidadas en refs de `EIoVx`.
   - **Cuenta/Fecha**: `TransactionFormFieldButton` (compartido) no tenía ícono ni chevron pese a que el componente real `wOlOA` siempre los tiene — corregido, beneficia a ambas features. `ScheduledPaymentDateField` (antes un `InputDecorator` genérico) reconstruido sobre ese mismo botón.
   - **Etiquetas**: `ScheduledPaymentTagChip`/`ScheduledPaymentTagPickerSheet` (duplicados) borrados; ambas features usan `TransactionFormTagChip`/`TagFilterSheet`/`NewTagSheet`, reconstruidos para coincidir con `FL1gK`/`NazyV` (antes `CheckboxListTile`/`TextField` genéricos). Se corrigió además un bug real de layout (no de diseño): el chip "Nueva" se estiraba a ancho completo dentro del `Wrap` por un `Container(alignment:...)` sin ancho explícito — fix con `IntrinsicWidth`. En Pencil, 12 instancias sueltas de "Tags Row" consolidadas; `FL1gK` ganó el ícono "+" que el código ya tenía pero el diseño no.
   - **Monto**: `MoneyFormatter.formatSymbol` construía el string vía `NumberFormat.currency(locale:'es_CO')`, cuyo patrón ICU pone el símbolo al final ("100 COP" en vez de "$100") — reescrito para anteponer `$` manualmente, sin depender del locale. Corrige el monto en toda la app (Pagos Programados, Transacciones, Cuentas).
   - Efecto secundario menor: se quitó el texto "N subcategorías" del filtro de categoría de Transacciones (pedido explícito del usuario, no relacionado con Pagos Programados pero tocado en la misma ronda).
