...
- **Una transacción con `debtId` asignado se excluye de los totales de ingreso/gasto en gráficas/informes, igual que `transfer`** (ver `10-graficas-informes.md`): prestar o cobrar un préstamo mueve efectivo pero no es ingreso ni gasto real (no cambia el patrimonio neto, solo la mezcla entre activo líquido y compromiso). `categoryId` sigue siendo opcional en estas transacciones (ver `02-categorias.md`), solo para organización/filtros del propio usuario, nunca para el cálculo de saldo pendiente ni para los totales de reportes.
