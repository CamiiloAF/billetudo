...
- **Tabla nueva `DebtEntries`** (mixin `_SyncColumns`) para los asientos de **solo-deuda** (los de caja ya son `Transaction` con `debtId`): `debtId → Debts.id`, `kind` (`textEnum<DebtEntryKind>`), `amountMinor` (con signo: + aumenta la deuda, − la reduce), `entryDate`, `note` (opcional), y `rateBpsSnapshot` (tasa usada para el interés, opcional). El saldo se deriva; no se guarda.
  - **`DebtEntryKind` (4 valores, resuelto en implementación 2026-07-22):** `interestAccrual` (interés, +), `manualAdjustment` (reconciliación "actualizar saldo", con signo), **`payment`** (abono sin caja, toggle "No" de HU-02, −) y **`disbursement`** (desembolso sin caja, toggle "No", +). Los abonos/desembolsos **con** caja son `Transaction` con `debtId`; los `payment`/`disbursement` de acá son la versión **sin caja** (cambian la deuda pero no mueven ninguna cuenta). *(Corrige la inconsistencia previa: HU-02 introduce el evento sin-caja pero la lista de kinds solo tenía interés/ajuste.)*
