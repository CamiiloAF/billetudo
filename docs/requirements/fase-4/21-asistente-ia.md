...
- **El nombre del proveedor hoy está hardcodeado en el cliente** (`aiConsentHeadline`/`aiConsentBody` en `app_es.arb`/`app_en.arb` dicen "Google" literal). Es correcto para Fase A — Apple 5.1.2(i) exige nombrar al tercero real en la pantalla de consentimiento, así que no puede decir "un modelo de IA genérico" mientras el proveedor sea Google — pero significa que **migrar de proveedor exige una nueva versión de la app** solo para cambiar ese texto, aunque el broker del backend ya soporte el cambio con una variable de entorno (`AI_PROVIDER`).
  **Decisión pendiente para cuando se evalúe cambiar de proveedor**: mover el nombre del proveedor (y opcionalmente la URL de su política de privacidad) a un remote config — extender `ai_feature_flags` (o una tabla nueva) con una fila tipo `ai_provider_display_name`/`ai_provider_privacy_url`, expuesta por `my_ai_access_state()` o una RPC nueva de solo lectura, y que `AiConsentPage` arme el texto legal con ese valor en vez de con una clave ARB fija. Así una migración de proveedor es un `UPDATE` en Postgres, no un release. **No implementado todavía** — la app hoy solo tiene un proveedor (Gemini) y no vale la pena la complejidad hasta que haya una migración real en el horizonte. Cuando se haga, hay que re-auditar `docs/legal/politica-de-privacidad.md` §17.4 y `declaraciones-tiendas.md` igual (el remote config resuelve el copy in-app, no el trámite legal).
