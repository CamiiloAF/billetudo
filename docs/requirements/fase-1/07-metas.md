...
- **Metas funciona sin cuentas; solo la rama que mueve dinero exige una** (gate de `15-gate-cuenta.md`). Crear una meta, aportar y retirar en **seguimiento puro** (toggle "¿Mover dinero de una cuenta?" en No) no requiere ninguna cuenta activa — es la feature que mejor sobrevive a ese estado y no debe bloquearse entera. Lo que sí se bloquea es el toggle en **Sí**: sin cuentas activas se presenta explicado (no en gris mudo) y ofrece crear la cuenta ahí mismo, continuando después con el aporte. Mismo criterio para vincular `accountId` en HU-02.
