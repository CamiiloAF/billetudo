...
- Si las cuentas seleccionadas tienen monedas distintas, no se muestra un total sumado del periodo (sumar monedas distintas como si fueran la misma unidad es incorrecto); se muestra el total desglosado por cuenta/moneda. Conversión a moneda base queda fuera de alcance de esta HU — ver `12-multi-moneda.md`.
...
- **Sin cuentas activas no se abre ningún formulario de esta feature** (ver `15-gate-cuenta.md`). `Transactions.accountId` es NOT NULL, así que registrar gasto, ingreso o transferencia exige al menos una cuenta activa (`deletedAt IS NULL AND tombstonedAt IS NULL`); la **transferencia exige dos**. No es un caso de usuario nuevo únicamente: cualquiera puede quedarse sin cuentas archivándolas todas (`01-cuentas.md` HU-08). El comportamiento es un **puente, no un muro** — el FAB y las acciones siguen visibles y tocables, y al activarlas se ofrece crear la cuenta en el momento y **continuar automáticamente al formulario original**. Prohibido deshabilitar el FAB en gris. Aplica también al entrar por ruta directa/deep link.
