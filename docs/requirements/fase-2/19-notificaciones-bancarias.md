...
**Decisión (2026-08-27, revisada el mismo día tras ver el diseño):** se ofrece de forma **contextual, después de que el usuario registra su primer gasto a mano** — no en el onboarding.

- **Por qué salió del onboarding:** la pantalla de explicación es la más densa de la feature (qué se lee, qué se guarda, qué sincroniza, qué va a decir el diálogo del sistema). Metida en el onboarding es demasiada carga para alguien que acaba de instalar la app, y rompe el tono del resto del flujo inicial. Además, pedir el permiso más invasivo del teléfono antes de que el usuario le tenga confianza a la app es la peor conversión posible.
- **Por qué contextual:** justo después de teclear un gasto a mano, el usuario acaba de sentir la fricción que esta feature resuelve. El consentimiento ahí es informado, no un trámite más del arranque.
- La entrada permanente en **Ajustes** sigue siendo obligatoria (HU-09) y es el camino para quien rechace el ofrecimiento contextual o quiera activarlo después.

**Regla de copy — no prometer cobertura que la app no puede garantizar (obligatoria):** el ofrecimiento contextual **no puede** redactarse como "¿quieres que esto aparezca solo?" ni equivalentes. La app no captura todo y nunca podrá: no hay notificación para un pago en efectivo, hay bancos fuera del catálogo, y el sistema puede matar el servicio (HU-09 lo asume explícitamente). Un copy que promete automatización total se rompe con el primer gasto que no aparece, y el usuario concluye que la feature está dañada.
- El copy debe enunciar la **condición**: *"Cuando tu banco te avise de una compra, te la dejo lista para confirmar"*. Dice qué pasa y bajo qué condición, sin prometer el resto.
- Es coherente con el título ya elegido para el explicador, "Leer los avisos de tu banco" — descriptivo, no promesa — y con la mitigación de política de Google Play de más abajo, que prohíbe construir la propuesta de valor sobre "tus gastos se registran solos".
...
**Decisión (2026-08-27):** catálogo **cerrado**, sin vía de solicitud en esta fase — solo apps del catálogo curado. Es la postura más defendible ante Google Play y ante el usuario ("solo miramos bancos"); el banco pequeño que quede fuera se atiende en una futura ampliación de catálogo, no con un formulario en el lanzamiento.

**Decisión (2026-08-27):** reglas **empaquetadas en la app** (asset local) — coherente con "sin backend, costo $0" y con que la feature siga 100% local desde el día uno. Corregir o añadir un banco exige una release y esperar a que el usuario actualice; se acepta ese costo a cambio de no introducir una dependencia de red en el camino de captura.

**Catálogo de emisores del lanzamiento (decisión 2026-08-27, revisada 2026-09-10):** Nu (Nubank), Nequi y **Google Wallet / Google Pay** — este último usa el mismo mecanismo sin costo adicional de implementación y captura pagos NFC que la app del banco a veces no notifica por separado, y es además el emisor de menor riesgo de política (es una app de Google, no un banco de terceros). Es el catálogo que HU-03 implementa hoy en `assets/capture/issuer_rules.json` y que expone la pantalla de activación de emisores.

**Bancolombia, Davivienda, Daviplata y BBVA Colombia quedan pospuestos (decisión 2026-09-10):** siguen siendo la ampliación de mayor penetración en es-CO y el objetivo natural del catálogo, pero de momento no van — no se han recolectado ni validado ejemplos reales de sus formatos de notificación (ver el pendiente de HU-03 más abajo), así que su interruptor no se ofrece en la UI hasta que tengan reglas validadas. Se retiran del catálogo cerrado activo, no se eliminan del plan: al validar el primero de ellos, se reactiva su fila en la pantalla de emisores y se documenta aquí.

**Apple Pay no aplica y no es una alternativa evaluable** (2026-08-27): esta feature es solo Android (ver "Alcance por plataforma") y Apple Pay no existe en Android. En iOS ninguna app de terceros puede leer notificaciones ajenas, así que no hay variante posible de esta idea en esa plataforma — la captura en iOS es voz (`17-captura-voz.md`) y OCR (`18-captura-ocr.md`). **Pendiente antes de escribir las reglas de parseo:** confirmar el `packageName` real de cada app en Google Play y capturar ejemplos reales (o representativos) del texto de sus notificaciones de compra/pago por emisor, porque el formato varía entre bancos y entre tipos de movimiento (compra con tarjeta, transferencia recibida, pago de servicio) — sin eso no se puede escribir ni probar el regex de HU-03. No copiar reglas de otra app: cada notificación bancaria tiene su propio formato de texto y hay que verificarlo contra la app real, no asumirlo.
...
**El parseo corre en Kotlin, dentro del servicio nativo (decisión 2026-08-27).** `NotificationListenerService` recibe notificaciones **con la app cerrada y sin motor Flutter vivo**; combinado con la retención cero (abajo), que exige descartar el texto crudo en el instante en que llega, el parseo debe ocurrir en el proceso nativo. El servicio extrae los campos, descarta el texto y persiste solo lo estructurado.

- **Consecuencia asumida:** HU-03 es la única lógica de negocio del proyecto que **no vive en `domain/` de Dart**, contra la regla general de `CLAUDE.md`. Se acepta a propósito: es la única opción que cumple retención cero de forma limpia y demostrable. Las alternativas se descartaron — un isolate Dart headless tendría que escribir en Drift sobre el `sqlite_async` que administra PowerSync (`drift_sqlite_async` 0.3.1, pre-1.0), territorio no explorado donde un fallo es silencioso; y bufferear el texto crudo para parsearlo al abrir la app **está prohibido** por la retención cero (persistir "hasta que abran la app" es persistir).
- **Mitigación obligatoria — fuente única de reglas + motor espejo:** las reglas viven en un solo `assets/capture/issuer_rules.json`, leído por Kotlin en producción. El **mismo** JSON alimenta una implementación espejo en Dart (`parse_bank_notification.dart`, dominio puro) que existe para poder probar la batería de ejemplos reales con `flutter test` y para el camino de Fase 4. Se duplica el **motor**, nunca las **reglas**: una regla que solo exista en uno de los dos lados es un bug.
- **Vacío de cobertura conocido:** ningún revisor del repo cubre Kotlin. El `IssuerFilter` (filtrado por `packageName` antes de leer `extras`) y el manifest exigen revisión humana explícita.
...
**La nota sugerida se compone de campos extraídos, nunca del texto crudo (decisión 2026-08-27).** La `note` de la transacción se arma **únicamente** con fragmentos que una regla identificó explícitamente como un campo (comercio, pista de tarjeta, tipo de movimiento) — por ejemplo `EXITO CALLE 80` o `EXITO CALLE 80 · tarjeta *1234`. **Está prohibido** usar como nota el contenido literal de la notificación, completo o recortado; en particular, el atajo de "tomar el mensaje y quitarle el monto" es retención cruda disfrazada y no es aceptable. Motivo: la `note` de una `Transaction` **sincroniza a Supabase** como cualquier otro campo, así que volcar ahí el texto de la notificación equivaldría a subir a un servidor el contenido literal de las notificaciones del teléfono — exactamente lo que la retención cero (abajo) y la política de privacidad prometen que no ocurre. La regla que separa lo permitido de lo prohibido: **cada pedazo de la nota debe provenir de un campo que el parser identificó como tal**, no de un fragmento del mensaje que sobró. Cuando se agrupan capturas de billetera y banco (HU-07), la nota puede tomar el mejor comercio de las dos, sujeto a esta misma regla.
...
- **Decisión (2026-08-27, refinada tras ver el diseño):** además de listarse en la bandeja, cada captura pendiente aparece como **fila "fantasma"** en el listado de transacciones de la cuenta sugerida (`../fase-1/03-transacciones.md`), **fijada al inicio del listado** — no mezclada cronológicamente. Van todas juntas arriba, antes del primer grupo de día. Motivo: intercaladas por fecha se pierden dentro de un historial largo, que es justo lo contrario de lo que la fila fantasma busca (que no se te olvide confirmarlas). La fila lleva una marca visual que la distingue de un movimiento confirmado, y tocarla abre el mismo flujo de despacho de HU-05.
  - Al ir fijadas arriba y no dentro de un grupo de día, **no participan de ningún total diario**, lo que refuerza estructuralmente la regla de que no suman. El encabezado de su bloque debe dejar claro que son propuestas pendientes, no movimientos del día. **Sigue sin ser una `Transaction`** y sigue sin sumar al total del listado, al saldo de la cuenta ni a ningún cálculo — es una fila derivada de `PendingCaptures` que la UI mezcla en el momento de renderizar, no un dato nuevo persistido en `Transactions`. Si la cuenta sugerida no existe o cambia, la fila deja de aparecer en ese listado (sigue en la bandeja).
  - **Motivo de esta decisión:** se evaluó crear la captura ya como `Transaction` real marcada "pendiente" para lograr esta visibilidad sin ir a la campana. Se descartó: el parseo puede fallar (tipo/cuenta por defecto, categoría nunca inferida — HU-03) y una `Transaction` real ya cuenta para saldo/presupuesto/metas/gráficas aunque esté mal; y rompe la deduplicación de HU-07, que hoy resuelve el posible duplicado *antes* de crear cualquier movimiento real — con creación automática habría dos movimientos de dinero simultáneos (el manual y el automático) hasta que el usuario lo note. La fila fantasma da la misma visibilidad sin ese riesgo.

**Decisión (2026-08-27):** la bandeja vive detrás de la **campana del Home**. Ubicación real verificada: el botón está en `lib/features/home/presentation/widgets/home_header.dart:106-113` (`IconButton` con `LucideIcons.bell` y callback `onBellTap`), y el `ComingSoonSheet` que hoy abre vive en `lib/features/home/presentation/pages/home_page.dart:137-144` (`_openBellSheet`, cableado en `:270`). El badge de conteo tiene precedente directo en `home_state.dart:96` (`pendingScheduledCount`) — se sigue ese mismo patrón, no se inventa mecanismo.

**La campana abre una PANTALLA COMPLETA, no una hoja inferior (decisión 2026-08-27, revisada tras ver el diseño).** El primer diseño la resolvió como bottom sheet y quedó claro que no cabe: la campana aloja **dos** contenidos —capturas pendientes por confirmar y avisos/recordatorios (`fase-3/22-centro-notificaciones.md`)— y en una hoja compiten por el mismo espacio limitado, forzando a elegir cuál se sacrifica.
- **Criterio de diseño:** de un solo vistazo, **sin scroll**, el usuario debe poder ver lo que tiene pendiente **más** los avisos. La pantalla completa es lo que hace eso posible.
- Los avisos y recordatorios son, en general, **más importantes** que las capturas por confirmar (un cobro que vence tiene consecuencia; una captura sin despachar no afecta ningún número). La jerarquía de la pantalla debe reflejarlo — pero ambas secciones tienen que caber, que es justo el punto.
- Reemplaza el `ComingSoonSheet` actual por una **ruta propia**, no por otra hoja. El `ComingSoonSheet` de `home_page.dart:137-144` desaparece.
- Sigue aplicando la estructura de **secciones** ya decidida: cada sección se renderiza solo si tiene contenido, y la de avisos queda lista para `22-centro-notificaciones.md` sin rehacer la pantalla. Reusa una superficie que ya existe, encaja con la metáfora de "cosas que requieren tu atención" y es coherente con `fase-3/22-centro-notificaciones.md`, que ya proyecta esa misma campana como bandeja de insights terminales (recordatorios de vencimiento, avisos de presupuesto). Al construir ambas features hay que coordinar el diseño de la campana como contenedor **unificado** de secciones (capturas pendientes, notificaciones del centro), no dos hojas independientes compitiendo por el mismo ícono.
...
- Campos pre-llenados: `amountMinor`, `currency`, `date`, `accountId` (si se pudo sugerir), `note` (compuesta solo de campos extraídos — ver la regla de nota sugerida en HU-03, nunca el texto crudo de la notificación), `type`. **`categoryId` viene vacío** salvo aprendizaje (HU-06), y sigue siendo **obligatorio** para `income`/`expense` según HU-01 de `03-transacciones.md` — la app no registra un gasto sin categoría solo porque venga de una notificación.
...
- **Deduplicación entre emisores distintos — billetera vs. banco (decisión 2026-08-27).** Un pago con Google Wallet genera **dos** notificaciones de apps distintas: la de Google Wallet y la del banco emisor de la tarjeta usada. Con la regla anterior (que exige mismo emisor) se colarían como dos capturas independientes. Por eso el agrupamiento **no puede condicionarse a que el `sourcePackage` coincida**: dos capturas de emisores distintos, mismo `amountMinor` y misma `currency`, dentro de una ventana **muy corta** (minutos — el pago NFC y el aviso del banco son prácticamente simultáneos), se presentan **agrupadas como un solo movimiento propuesto**, no como dos.
  - Al agrupar, la captura de la **app del banco** es la que manda para la pista de cuenta (`accountHint`), porque identifica la tarjeta real; la de la **billetera** suele traer mejor nombre de comercio. La agrupación puede tomar el mejor campo de cada una, pero **nunca inventa** un dato que ninguna de las dos trajo.
  - Sigue aplicando la regla general: **la app nunca fusiona ni descarta automáticamente sin mostrar**. Agrupar en la UI no es confirmar; el usuario ve un solo movimiento propuesto y sigue teniendo que despacharlo (HU-05).
  - Este caso es de **alta confianza** (misma cantidad, mismo instante, emisores complementarios conocidos), a diferencia de la deduplicación contra transacciones manuales, que es heurística. Puede tratarse con una ventana más estricta y agrupación visual directa, mientras que el caso manual solo se marca como "posible duplicado".
...
- **Una captura pendiente no es dinero.** No afecta saldos, presupuestos (`../fase-1/06-presupuestos.md`), metas, deudas, gráficas (`../fase-1/10-graficas-informes.md`) ni "disponible para gastar", en ningún estado intermedio — **tampoco cuando se muestra como fila fantasma en el listado de transacciones** (HU-04, decisión 2026-08-27). Verse en el listado es una decisión de UI, no una promoción a `Transaction`.
...
> **Verifica el número antes de escribir la migración, no lo copies de aquí.** Al **2026-08-27** el working tree está en **`schemaVersion` 30** (`lib/core/database/app_database.dart:968`, con bloques `if (from < 29)` y `if (from < 30)` ya presentes). Con ese estado, **esta feature tomaría 30 → 31 y OCR 31 → 32**. (La estimación anterior de este doc — 28 → 29 — quedó obsoleta en diez días, que es justo la razón de esta advertencia.) Lee el `schemaVersion` vigente en el momento de implementar y toma el siguiente libre. Dos features reclamando la misma versión producen una base local que se cree migrada sin estarlo, y ese fallo es silencioso.
