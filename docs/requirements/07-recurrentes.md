# Feature: Transacciones recurrentes / pagos planeados

**Nivel:** 0 (gratis, ilimitado, sin anuncios)
**Tabla Drift:** `Recurrings` (`lib/core/database/app_database.dart`)

## Contexto

Plantillas de transacciones que se repiten (suscripciones, arriendo, nómina) con frecuencia configurable. Alimentan además los recordatorios de vencimientos (notificaciones locales, Fase 2) y el cálculo de "disponible para gastar" al proyectar compromisos futuros.

## Historias de usuario

### HU-01 — Crear una plantilla recurrente
...
- Campos: `accountId` (obligatorio), `categoryId` (opcional), `amountMinor` (obligatorio), `currency`, `type` (income/expense/transfer), `note` (opcional), `frequency` (`RecurFrequency`: daily/weekly/monthly/yearly), `interval` (cada cuántas unidades de frecuencia, default 1), `nextDate` (obligatoria), `endDate` (opcional).
- No hay límite de recurrentes activas (Nivel 0).
...
**Criterios de aceptación:**
- La transacción generada tiene `source = recurring` y `recurringId` apuntando a la plantilla.
...
Como usuario quiero poder revisar una transacción recurrente antes de que se aplique definitivamente (ej. si el monto de este mes varía), para ajustar montos variables como servicios públicos.
...
Como usuario quiero ver una lista de mis próximas transacciones recurrentes ordenadas por fecha, para anticipar mis compromisos financieros.
...
### HU-05 — Editar y eliminar recurrente
Como usuario quiero modificar el monto, frecuencia o fecha de una plantilla recurrente, o eliminarla si ya no aplica (ej. cancelé una suscripción).

**Criterios de aceptación:**
- Editar la plantilla no modifica transacciones ya generadas en el pasado, solo las futuras.
- Eliminar detiene la generación futura; es borrado lógico (`deletedAt`), y las transacciones ya generadas mantienen su `recurringId` como referencia histórica.

## Reglas de negocio y edge cases

- Una recurrente de `type = transfer` requiere `transferAccountId` igual que una transacción normal (ver `03-transacciones.md`).
