...
- Las **transferencias** (`type = transfer`) no cuentan, **salvo** las marcadas `countsInBudget` (ver Reglas de negocio).
...
- Las **transferencias** (`type = transfer`) no cuentan para el progreso de ningún presupuesto (no son gasto real; evitan doble conteo con el pago de tarjeta — ver `03-transacciones.md`). Esto vale también para presupuestos con alcance de cuenta (el pago de la tarjeta desde otra cuenta no consume el presupuesto de la tarjeta).
  - **Excepción — transferencia presupuestable (`countsInBudget = true`):** entra al progreso usando el **mismo mecanismo de alcance** que cualquier gasto, con su `categoryId` (obligatoria en ese caso). Cuenta como **gasto** para todo presupuesto cuyo alcance incluya la cuenta **origen** — **implementado** (Fase B1, `BudgetsLocalDatasource.watchExpenses`): el lado origen se pliega como una fila de gasto más, así que sigue exactamente el mismo `BudgetProgressCalculator` que un gasto normal, sin regla nueva. El diseño también contempla que cuente como **ingreso** para todo presupuesto cuyo alcance incluya la cuenta **destino**, pero ese lado queda **pendiente**: el dominio de presupuestos hoy solo modela un total de ingreso global (`ZeroBasedSummary`), no un concepto de "ingreso que reduce el gasto de *este* presupuesto según su alcance" — implementarlo requeriría un mecanismo nuevo, no una extensión del que ya existe. Si ambas cuentas caen en el mismo presupuesto, el efecto neteo (gasto en origen cancelado por el ingreso en destino) tampoco aplica todavía por lo mismo. Ver `docs/plan-cuentas-tipos-y-transferencias-presupuestables.md` §3.
  - Ojo: la simetría origen/destino es **exclusiva de presupuestos** (que tienen alcance por cuenta). En gráficas e informes, que son globales, solo cuenta el lado origen — ver `10-graficas-informes.md` §Reglas de conteo.
