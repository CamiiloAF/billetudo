...
- **Deudas consume esta feature para las cuotas** (ver HU-03 de `08-deudas.md`): una deuda puede enlazar una plantilla por `debtId` para su cuota. La `Transaction` generada hereda ese `debtId`, así baja la cuenta **y** reduce la deuda. Reparto de responsabilidades para no generar ruido: **configurar/editar** la cuota vive en Deudas; **confirmar/omitir/posponer** la ocurrencia que vence vive acá (la bandeja única de vencimientos). En la **lista** y en la **tarjeta** un **badge/banner** identifica que la plantilla corresponde a una deuda; en el **detalle** del pago, un **card** muestra cuál deuda es y navega a su detalle al tocarlo. Editar la plantilla de una cuota **deep-linkea de vuelta a la deuda** (su hogar), no se edita como plantilla suelta.
...
- **Columna `debtId` nullable** (`text().nullable().references(Debts, #id)`) para enlazar una plantilla a la deuda cuya cuota representa (ver `08-deudas.md`, HU-03). Aditiva; habilita el cross-link y que la transacción generada herede el `debtId`.
- Subir `schemaVersion` (hoy 10 → 11) y escribir la migración `onUpgrade` (crear la tabla puente + añadir `debtId`; no hay datos que migrar, es aditiva). Mantener paridad en Supabase/PowerSync. Ejecutar vía `/drift-schema-change`.
