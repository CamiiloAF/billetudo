...
2. **Ningún paso es obligatorio: la primera cuenta se propone pre-llenada, pero se puede omitir.** No se obliga al usuario a crear nada para entrar a la app — obligarlo castiga sobre todo a quien **ya tiene sus datos en la nube** y solo quiere iniciar sesión (ver #5). La fricción se ataca por pre-llenado, no por imposición: nombre "Efectivo", tipo `cash`, moneda según la región del dispositivo, saldo inicial 0 — un toque en "Listo" y sigue; o "Omitir" y entra igual. Se descartó crear una cuenta implícita al omitir (aparece un dato que el usuario no creó y no entiende de dónde salió).
...
- **Reutiliza el formulario de `01-cuentas.md` HU-01** (mismo caso de uso `CreateAccount`, mismas validaciones, mismos widgets de selección), **simplificado por defecto**: se muestran nombre, tipo, moneda y saldo inicial. Se ocultan `institution`, número de cuenta/`last4`, `interestRateBps`, ícono y color — todos opcionales y editables después desde Cuentas. **Nada de una segunda implementación del formulario**: si el diseño exige una variante compacta, es una variante del mismo widget, no un formulario paralelo.
- **Pre-llenado con valores por defecto razonables**, todos editables:
  - nombre: "Efectivo" (localizado);
  - tipo: `cash`;
  - moneda: derivada de la **región del locale del dispositivo** (CO → COP, MX → MXN, AR → ARS, CL → CLP, PE → PEN, ES → EUR, US → USD…). Si la región no resuelve a una moneda conocida, el fallback es **USD** y el selector se abre resaltado para que el usuario elija. La moneda **siempre es editable** aquí — nunca se fija en silencio.
  - saldo inicial: 0.
...
- El usuario **puede crear más de una cuenta** en este paso ("Agregar otra"), y las ya creadas se listan a la vista. También puede quedarse con una sola y seguir.
