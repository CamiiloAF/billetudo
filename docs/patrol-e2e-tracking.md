...
| Autenticación | 🟡 Parcial | 2026-09-10 | ✅ (`integration_test/auth_patrol_test.dart`) | `/pr-review 37`, emulator-5556 dedicado, 2 corridas idénticas | ❌ 4/5, reproducible en 2/2 corridas limpias (no flaky). "HU-07 paso 1: confirmar sin backend cableado cae al estado de error, nunca crashea la app" — `Found 0 widgets with text "No pudimos eliminar tu cuenta"`. No relacionado con PR #37 (login Apple/FAB/avatar no toca el flujo de eliminar cuenta). `docs/dev-runs/patrol-e2e-findings-2026-09-10.md` documentó este fallo como "nuevo, ver abajo" pero nunca lo desarrolló — sigue sin causa raíz confirmada, candidato: el estado de error de `DeleteAccount` al fallar la llamada real a Supabase podría no montarse a tiempo para el `pumpAndSettle`, patrón similar al de HU-03/HU-05 de Transacciones. Histórico (2026-07-21): ✅ 5/5. |
...
| Inicio / Dashboard | 🟡 Parcial | 2026-09-10 | ✅ (`integration_test/home_patrol_test.dart`) | `/pr-review 37`, emulator-5556 dedicado, 2 corridas (una con ventanas de espera 3-5x más largas) | ❌ 3/4, reproducible igual en ambas corridas (no es timing — alargar `_pumpUntilFound`/`dismissAutoTutorialIfShown` no cambió el resultado). "HU-01: Presupuestos y Metas abren sus features reales" — tras tapear "Metas", `GoalsListPage` nunca aparece en el árbol, sin ningún error de Flutter en consola. **No es una regresión de PR #37** (`git diff d810d9b5..HEAD` confirma que el PR no toca `app_router.dart`/`home_shell_page.dart`/`goals/`/`budgets/`/`tutorials/`, solo el propio test). Ver `docs/dev-runs/patrol-e2e-findings-2026-09-10.md` § "Corrección posterior" — el fix que esa misma sesión había dado por bueno (4/4) no es confiable; pendiente de investigar con logcat en vivo o el reporte HTML de Patrol, no por lectura de código (ya se intentó dos veces sin éxito). Histórico (2026-07-21): ✅ 4/4. |
