...
| Deudas | 🟡 Parcial | 2026-07-22 | ✅ (`integration_test/debts_patrol_test.dart` + `integration_test/debts_installment_patrol_test.dart`) | corrida `patrol-e2e-runner` 2026-07-22 (emulator-5554, primera corrida en device) | 🟡 **9/17** (núcleo 6/12 en 2m18s, cuotas 3/5 en 2m30s). Infra OK (ya no reaparece el bug `startApp`/PowerSync). **Cluster principal:** los 3 abonos desde la hoja (`_submitAbono`, líneas 303/216/379) no reflejan el saldo nuevo, mientras "enlazar movimiento existente" SÍ baja el saldo → revisar submit de la hoja de abono / stream del detalle, no la ruta de enlace. Arrastran HU-07 saldada (`:500`, no llega a 100%) y Ledger completo (`:609`, $800). 2 fallas en helper `_scrollUntilVisible` (`:216`: "Too many elements" finder ambiguo en abono-sin-caja; "No element" en interés auto). Cuotas: `ScheduledDebtChip` no aparece en PP (`:300`) y deep-link editar cuota no muestra nombre de deuda (`:412`) — cross-link inverso y confirmación de ocurrencia sí pasan. Para flutter-dev (bug de aplicación del abono directo, sospecha fuerte) + qa-automator (finders de scroll / `Key`s del chip y deep-link). |
