...
| Metas, Improvement | ⬜️ N/A | — | ❌ | — | Sin implementación en `lib/features/` (solo `.gitkeep`) — lienzo en blanco, no aplica escribir suite Patrol todavía. Nota: la fila original de esta tabla también listaba "Reportes" aquí, pero `integration_test/reports_patrol_test.dart` ya existe con suite propia — no tocado en esta edición, fuera del alcance de la tarea de Captura; vale una limpieza aparte. |
