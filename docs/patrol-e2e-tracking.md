...
| Autenticación | ✅ Verde | 2026-09-11 | ✅ (`integration_test/auth_patrol_test.dart`) | `/pr-review 37`, emulator-5554 dedicado, 2 corridas consecutivas | ✅ **5/5 estable en 2 corridas seguidas.** Causa raíz confirmada del fallo del 2026-09-10 (🟡, `Found 0 widgets with text "No pudimos eliminar tu cuenta"`): **no era un bug de producto, era un escenario de test desactualizado.** `AuthRepositoryImpl.deleteAccount` tiene un short-circuit deliberado (`if (!_current.isSignedIn) return const Right(unit)`) — sin sesión no hay nada que borrar en la nube, así que paso 1 siempre tiene éxito y avanza directo a paso 2, nunca llega al estado de error. El escenario asumía (documentado en el propio header del archivo, ya desactualizado) que el paso 1 "actualmente sin implementar" siempre fallaba — dejó de ser cierto cuando se implementó el short-circuit, sin actualizar este escenario. La ruta de error real ya está cubierta sin backend por `test/features/auth/presentation/cubit/delete_account_cubit_test.dart` + los goldens de `confirm_delete_account_sheet`. Fix: escenario reescrito para afirmar lo que sí es alcanzable end-to-end (paso 1 sin sesión → avanza a paso 2 con la copia "nunca iniciaste sesión", nunca al sheet de error). Histórico (2026-07-21): ✅ 5/5; (2026-09-10): 🟡 4/5. |
...
| Inicio / Dashboard | ✅ Verde | 2026-09-11 | ✅ (`integration_test/home_patrol_test.dart`) | `/pr-review 37`, emulator-5554 dedicado, 3 corridas consecutivas | ✅ **4/4 estable en 3 corridas seguidas** (8s/3s el escenario de Metas). Causa raíz confirmada del fallo del 2026-09-10 (🟡, ver `docs/dev-runs/patrol-e2e-findings-2026-09-10.md` § "Resuelto de verdad esta vez"): el minitutorial de Presupuestos/Metas interfería con el tap a la siguiente pestaña, y `dismissAutoTutorialIfShown` no lo atrapaba de forma confiable pese a estar pensado para eso (ni con ventanas de espera 3-5x más largas). Fix real: `AppSettings.showHelpOnSectionEntry` se apaga explícitamente al inicio de este escenario (`getIt<AppSettingsCubit>().setShowHelpOnSectionEntry(enabled: false)`) — este escenario prueba navegación entre pestañas, no el minitutorial. **Riesgo de producto sin confirmar, distinto de la flakiness del test:** si `TutorialGateCubit.evaluate()` tarda lo suficiente en un dispositivo real, un usuario que toque "Presupuestos" y de inmediato "Metas" podría ver el minitutorial de Presupuestos justo cuando esperaba llegar a Metas — mismo mecanismo que rompía el test. No confirmado fuera del harness; candidato para que `flutter-dev` lo evalúe. No es una regresión de PR #37 (`git diff d810d9b5..HEAD` confirma que el PR no toca `app_router.dart`/`home_shell_page.dart`/`goals/`/`budgets/`/`tutorials/`). Histórico (2026-07-21): ✅ 4/4; (2026-09-10): 🟡 3/4. |
| Transacciones | 🟡 Parcial | 2026-09-10 | ✅ (`integration_test/transactions_patrol_test.dart`) | `/pr-review 37`, emulator-5556 dedicado, corrida completa (12/12 ejecutados, sin corte de infraestructura) | ❌ 8/12 — HU-03, HU-05, "Fase B1+B2" e "Issue #7" siguen fallando exactamente igual que en `patrol-e2e-findings-2026-09-10.md` (preexistente, investigado y sin causa raíz confirmada dos veces — 25 ago y 10 sep). **HU-07 ("crear una etiqueta nueva al vuelo"), el escenario más relevante al fix de colapso de teclado de PR #37, pasó limpio** — sin regresiones nuevas de ese PR en esta suite. Histórico (2026-07-24): ✅ 10/10 — regresó a 🟡 sin que se identifique el commit exacto que lo rompió entre el 24 jul y el 25 ago (fuera del alcance de esta verificación puntual). **3 fallos de infraestructura de test cerrados el 2026-07-24, ninguno bug real de `lib/`:** (1) `_createAccountScopedBudget` buscaba `find.text('Cuentas')` pero `BudgetNavField` renderiza label+valor en un solo `Text` interpolado — fix a `find.textContaining`; de paso destapó que `_goToTransactions` (usa `push()`) llamado 2 veces sin popear duplica la key de página y revienta `Navigator` — se agregó `_returnToTransactions` (usa `go()`) para la segunda entrada. (2) Seed de HU-04 caso 2 vía `TransactionsCompanion.insert` no seteaba la columna nueva `countsInBudget`; como `powersync_schema.dart` la declara sin default SQL (el `withDefault(false)` de Drift solo existe en el DDL Dart), el insert dejaba `NULL` físico y el mapper generado revienta con "Null check operator used on a null value" al leer la fila — no era timing/flaky, era determinístico. Fix: `countsInBudget: const Value(false)` explícito en el seed. **Riesgo latente sin corregir** (mismo patrón de insert, fuera del alcance de esa tarea): `categories_patrol_test.dart` y `debts_patrol_test.dart` también insertan `Transaction` sin `countsInBudget`. (3) `_enterAmount` comparaba contra `find.text('\$$whole')` sin agrupar miles, pero la UI agrupa con puntos (es-CO) desde \$1.000 — el finder nunca matcheaba y el retry interno triple-tapeaba cada dígito, escalando el monto exponencialmente; fix: comparar con `MoneyFormatter().formatSymbol(...)`, el mismo formateador que usa la UI. Ningún test anterior a Fase B1+B2 había tipeado montos ≥ \$1.000, por eso no se había visto. **Bug de `NewTagSheet` cerrado** (2026-07-21): `BottomSheetBase` ya pad-ea su `child` por el inset del teclado; `new_tag_sheet.dart` agregaba un segundo `Padding` idéntico, restando `viewInsets.bottom` dos veces y desbordando el `Column` de alto fijo cuando el timing de la animación del teclado dejaba justo poco margen (por eso era intermitente). Fix de `flutter-dev`: quitó el padding duplicado y envolvió el contenido en `SingleChildScrollView` como red de seguridad real. |
...
| Transacciones | ✅ Verde | 2026-09-11 | ✅ (`integration_test/transactions_patrol_test.dart`) | corrida contra PR #39 (`feat/transacciones-period-nav-presupuesto`), commit final `5d8a9afc` | **Corrida 2026-09-11 contra PR #39:** 8/12 en la primera pasada — 2 fallos eran **regresión real de este PR** (chip "Todas"/"Limpiar" solo-ícono, `find.text` desactualizado) y ya se corrigieron y verificaron en verde en device (commit `b79a4974`); los otros 2 (HU-03 transferencia, `_createAccountScopedBudget` en Fase B1+B2) son deuda preexistente ya documentada abajo (2026-07-24/2026-08-25), no relacionada con este PR, sin tocar. **Gap cerrado:** se agregó el escenario nuevo "Period Nav Bar: aplicar un filtro de Fecha no-default..." (commit `5d8a9afc`, 3/3 corridas estables) — cubre aparición condicional del `PeriodNavBar`, tap Prev/Next, y el estado `Next` deshabilitado en el borde del periodo, que no tenía ninguna cobertura e2e hasta ahora. Suite total tras esta ronda: 13 escenarios, 11 verdes, 2 con deuda preexistente sin tocar. Base histórica ✅ 10/10 estable en 2 corridas seguidas (9 previos + 1 nuevo: "Fase B1+B2: transferencia con toggle '¿Incluir en tu presupuesto?' cuenta como gasto del presupuesto de su cuenta origen"). **3 fallos de infraestructura de test cerrados el 2026-07-24, ninguno bug real de `lib/`:** (1) `_createAccountScopedBudget` buscaba `find.text('Cuentas')` pero `BudgetNavField` renderiza label+valor en un solo `Text` interpolado — fix a `find.textContaining`; de paso destapó que `_goToTransactions` (usa `push()`) llamado 2 veces sin popear duplica la key de página y revienta `Navigator` — se agregó `_returnToTransactions` (usa `go()`) para la segunda entrada. (2) Seed de HU-04 caso 2 vía `TransactionsCompanion.insert` no seteaba la columna nueva `countsInBudget`; como `powersync_schema.dart` la declara sin default SQL (el `withDefault(false)` de Drift solo existe en el DDL Dart), el insert dejaba `NULL` físico y el mapper generado revienta con "Null check operator used on a null value" al leer la fila — no era timing/flaky, era determinístico. Fix: `countsInBudget: const Value(false)` explícito en el seed. **Riesgo latente sin corregir** (mismo patrón de insert, fuera del alcance de esa tarea): `categories_patrol_test.dart` y `debts_patrol_test.dart` también insertan `Transaction` sin `countsInBudget`. (3) `_enterAmount` comparaba contra `find.text('\$$whole')` sin agrupar miles, pero la UI agrupa con puntos (es-CO) desde \$1.000 — el finder nunca matcheaba y el retry interno triple-tapeaba cada dígito, escalando el monto exponencialmente; fix: comparar con `MoneyFormatter().formatSymbol(...)`, el mismo formateador que usa la UI. Ningún test anterior a Fase B1+B2 había tipeado montos ≥ \$1.000, por eso no se había visto. **Bug de `NewTagSheet` cerrado** (2026-07-21): `BottomSheetBase` ya pad-ea su `child` por el inset del teclado; `new_tag_sheet.dart` agregaba un segundo `Padding` idéntico, restando `viewInsets.bottom` dos veces y desbordando el `Column` de alto fijo cuando el timing de la animación del teclado dejaba justo poco margen (por eso era intermitente). Fix de `flutter-dev`: quitó el padding duplicado y envolvió el contenido en `SingleChildScrollView` como red de seguridad real. |
| Presupuestos | ⚠️ 4/5, re-verificar | 2026-07-21 (última corrida estable) / regresión detectada 2026-09-11 | ✅ (`integration_test/budgets_patrol_test.dart`) | corrida `qa-automator` de hoy, 2 corridas consecutivas | **Corrida 2026-09-11 (refuerzo tras PR #39 de Transacciones, `lib/features/budgets/` no está en el diff de ese PR):** HU-13 falló (`find.textContaining('$0 de $1.000.000')` no encontrado justo tras crear el presupuesto) — sospecha razonable de que sea el edge case de `MoneyInputFormatter` ya documentado en la Nota 6 de abajo (reemplazo completo de texto interpretado como backspace), pero **no confirmado** (el monto usado no calza exactamente el patrón descrito). No es una regresión de Transacciones ni de este PR — rompe el "5/5 estable" que se sostenía desde 2026-07-21. **Pendiente: re-correr esta suite sola para confirmar si es flake puntual o regresión real antes de asumir cualquiera de las dos.** ✅ 5/5 estable en 2 corridas seguidas (última confirmación: 2026-07-21). Todas las 5 usan alcance "Todo" (global, HU-02) a propósito — un alcance por categorías necesita el catálogo de `category_seeds`, que se siembra desde Supabase y no es determinístico offline/en un proyecto de pruebas. Cubre HU-01/02 (crear), HU-09 (editar nombre+monto), HU-13 (ajustar monto solo el período actual — la regresión que cerró el commit `0c55978` — confirmando que el período siguiente conserva el monto original al navegar con el stepper), HU-10/11 (cerrar → histórico → reactivar) y HU-11 (eliminar con confirmación, incluida la ruta de "Cancelar"). **Posible edge case de `MoneyInputFormatter` encontrado, no confirmado como bug real de usuario** (ver Notas abajo) — reportado a `flutter-dev`, no corregido acá (fuera de alcance de `qa-automator`, solo lee/escribe bajo `test/`+`integration_test/`). |
| Pagos programados | 🟡 Parcial | 2026-09-11 | ✅ (`integration_test/scheduled_payments_patrol_test.dart`, 10 escenarios) | corrida `patrol-e2e-runner` 2026-09-11, máquina confirmada libre (load 17→3.5, sin otras sesiones de `patrol test` corriendo), worktree `feat/local-notifications`, commit `de3490f2`, emulator-5554, flavor `dev`, 2 corridas + 1 corrida diagnóstica con logcat | 🟡 **Cierre real de la regresión de HU-08, con un hallazgo de herramienta (Patrol, no de la app) sin resolver.** Secuencia completa de esta regresión: (1) 2026-09-10 temprano, 3 `RenderFlex overflowed` reales (`date_picker_sheet.dart:100`, `scheduled_payment_form_page.dart:181`, `scheduled_payment_reminder_sheet.dart:68`) — `flutter-dev` diagnosticó la causa raíz (`dismissSystemKeyboard` no esperaba a que el inset del teclado real llegara a 0 antes de abrir un sheet de altura fija) y lo corrigió en `lib/core/forms/keyboard.dart` (commit `2c1e34d2`); (2) esa misma corrida de más alto (verificando el fix de overflow) destapó una regresión nueva: el polling acotado de `dismissSystemKeyboard` (hasta 320ms sin ningún rebuild) creaba una ventana donde `pumpAndSettle()` podía "creer" que la UI ya se asentó antes de que la hoja de recordatorio existiera — `qa-automator` lo corrigió endureciendo el helper `_pickReminder` para esperar explícitamente al `ReminderOptionRow` en vez de confiar en `pumpAndSettle()` (commit `de3490f2`), y de paso corrigió una colisión de texto evitable en el escenario "Sin recordatorio". **Confirmado en esta corrida final: los 3 overflows siguen en cero, y 2 de los 3 escenarios de HU-08 (`configurar "3 días antes"` y `borrar con recordatorio`) pasan 3/3 estable** — la regresión completa (overflow + fragilidad del test) queda cerrada para esos dos. El tercero, `HU-08: cambiar a "Sin recordatorio"`, **no pudo certificarse por una anomalía del harness Patrol/AndroidX Orchestrator, no de la app**: en las 3 corridas el conteo final quedó en `Total: 9` (nunca 10), y una corrida diagnóstica con `--show-flutter-logs` + `adb logcat` confirmó que el escenario **sí corre y sí termina con éxito internamente** (`PatrolBinding: tearDown()... results: {..., "Sin recordatorio": success}`), pero el orchestrator lo descarta explícitamente (`"finished test ..., but it was not requested, so its status will not be reported back to the native side"`) sin ningún `FATAL`/ANR/excepción en logcat. Es determinístico (3/3) pero es un bug de instrumentación, no de `lib/`; no se investigó más a fondo (fuera del rol de `patrol-e2e-runner`, que no edita `test/`/`lib/`) — si se quiere cerrar del todo, aislar ese escenario en su propio archivo temporal para descartar interacción de conteo con el resto del bundle es el siguiente paso, tarea de `qa-automator`. **Único fallo real y no relacionado con este PR:** `HU confirmar un pago manualmente antes de su fecha (fix 81cb943)` sigue fallando 3/3 con `Found 2 widgets with text "Internet"` — ya documentado como preexistente en `docs/dev-runs/patrol-e2e-findings-2026-08-25.md`. **Veredicto: la regresión que bloqueaba este PR está cerrada; el PR #29 puede considerarse funcionalmente listo, con el detalle de instrumentación de "Sin recordatorio" y el fallo preexistente de "Confirmar ahora" como los únicos pendientes, ninguno atribuible al código de este PR.** (Historial resumido: 2026-09-10 temprano, pre-fix, 3/10 y 3/10 por los 3 overflows reales; 2026-09-10 tarde, post-fix de overflow pero con la regresión de `_pickReminder` sin corregir aún, 5/10 y 6/10; 2026-09-09, la corrida que dejó la suite en 10/10 antes de que HU-08 introdujera esta regresión — el detalle completo de esas 3 corridas queda en el historial de commits de este archivo, no se repite aquí para no romper la tabla.) |
...
| Onboarding | ✅ Verde | 2026-09-11 | ✅ (`integration_test/onboarding_patrol_test.dart`) | corrida `patrol-e2e-runner` 2026-09-11, emulator-5554, flavor `dev` | ✅ 2/2 corridas completas, 3/3 escenarios cada una (17s+7s+3s = 1m47s en la más rápida). Cubre HU-01/02/04 (primer arranque creando cuenta pre-llenada → registrar primer movimiento → Inicio), HU-02/04 (primer arranque omitiendo cuenta → Inicio con CTA "crear cuenta") y el nuevo AC 3 (`aceptacion-terminos-onboarding-pendiente.md`: "Ya tengo cuenta" abre la misma hoja de aceptación de términos antes de llegar a `LoginPage` — ya no es skip, existe y pasa). Una corrida previa el mismo día vio 2 fallos ("HomePage no encontrado" tras guardar el primer movimiento, y "LegalAcceptanceSheet no encontrado" tras "Ya tengo cuenta") — re-investigados aislando cada escenario 2x: ambos confirmados como **flake de infraestructura, no bug real**, causado por (a) contención real de emuladores compartidos entre sesiones/worktrees concurrentes corriendo Patrol en paralelo sobre el mismo device (ver memoria del proyecto `agentes-concurrentes-mismo-worktree-riesgo`) y (b) la mitigación de `patrol-e2e-findings-2026-08-25.md` (`rm -f integration_test/test_bundle.dart`) resultó insuficiente sola esta vez — también hace falta borrar `build/app/outputs/apk/androidTest` antes de cada corrida porque Gradle cachea el APK de instrumentación pese al bundle nuevo. Verificar `ps aux | grep "patrol test"` antes de correr esta suite para descartar contención. |
...
| Captura — widget de captura rápida | ✅ Verde (4/4) | 2026-09-11 | ✅ (`integration_test/capture_widget_shortcut_patrol_test.dart`) | escrita por `qa-automator` el 2026-09-10, `feat/capture-widget` | **Corrida real, 4/4 verde**, contra `dev` en `emulator-5554`. El primer intento crasheó `AndroidTestOrchestrator` con `IllegalArgumentException` tras 2 de 4 escenarios — causa: el nombre de un escenario tenía un `/` literal (`'HU-01/HU-02: ...'`), que el orquestador interpreta como separador de ruta al construir el archivo de resultado. Renombrado a "HU-01 and HU-02" (sin cambios de lógica); la suite completa pasó en la siguiente corrida. Cubre HU-01/HU-02 de `docs/requirements/fase-2/20-widget-captura-rapida.md`: (1) cold start con el shortcut "Gasto"/"Ingreso" ya pendiente → abre el form directo, sin que `HomePage` llegue a aparecer; (2) tap en caliente (`onNewIntent`/`singleTop`, app ya viva en Home) → empuja el form correcto encima; (3) un id de shortcut que este build no reconoce (app vieja / widget nuevo) no navega ni truena, en frío y en caliente. La suite mockea únicamente el canal de plataforma propio (`com.billetudo.app/capture_shortcuts`, vía `TestDefaultBinaryMessengerBinding` — mismo patrón que `capture_shortcut_repository_impl_test.dart` (unit) y `import_export_patrol_test.dart`'s `_mockFilePicker`/`_mockShareChannel`), nunca el resto del árbol de DI ni la BD; todo lo demás (`CaptureShortcutCubit`, `CaptureShortcutListener`, `go_router`, Drift real) es el código real. El código nativo (`QuickCaptureWidgetBridge.kt`, `MainActivity.kt`, el lado Swift) queda **fuera** de este mock por construcción — solo un tap real sobre el widget en un dispositivo/emulador lo ejercita; ver la nota de verificación humana al pie de este documento. `support/patrol_app.dart` ganó un parámetro opcional `beforeFirstFrame` en `startApp` (usado para sembrar una cuenta antes del primer frame, ya que los destinos del widget están detrás de `AccountGatedRoute` — HU-04 de `15-gate-cuenta.md` — y `CaptureShortcutCubit.start()` corre en el primer build, antes de que un seed post-`startApp` pudiera llegar a tiempo); es aditivo y no cambia el comportamiento de ninguna suite existente (`flutter analyze integration_test/` limpio tras el cambio). |
| Captura (bandeja de avisos bancarios) | ✅ Verde | 2026-09-09 | ✅ (`integration_test/capture_inbox_patrol_test.dart`) | corrida de verificación `patrol-e2e-runner` 2026-09-09, tras el fix de `flutter-dev`, emulator-5554, flavor `dev` | ✅ **6/6**, 1m 1s, sin flakiness. Cubre HU-05 (despacho feliz: tocar la captura en el Centro de avisos abre el formulario pre-llenado, confirmar la retira de la bandeja y crea la `Transaction` con `source=notification`), **la fila fantasma en Movimientos** (antes fallaba de forma determinística — ver detalle abajo, ahora corregido), el estado vacío del Centro de avisos ("Todo al día"), descartar con deshacer desde el snackbar, la rama de duplicado "Es otra compra" (continúa al form sin descartar) y la activación del permiso/catálogo de apps. Ningún bug abierto ni bloqueo de infraestructura pendiente en Captura. |
...
## Captura: fila fantasma nunca se muestra sin movimientos reales — RESUELTO (2026-09-09)

**Cerrado.** `flutter-dev` corrigió `TransactionsPage.build`: la rama `isEmpty` ahora
también monta `PendingCapturesListSlot` (justo encima del `TransactionsEmptyState`, igual
que ya hacía con el carrusel de saldo) — commit `b74f07d8`. Reverificado con
`patrol-e2e-runner`: los 6 escenarios de `capture_inbox_patrol_test.dart` pasan, incluida
"Fila fantasma en Movimientos" sin modificar su aserción original (seguía describiendo el
comportamiento correcto, nunca se debilitó para forzarla a pasar). Detalle histórico del
diagnóstico, conservado por su valor como referencia de cómo distinguir un test impaciente
de un bug real:
