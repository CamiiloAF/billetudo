...
| Pagos programados | 🟡 Parcial | 2026-09-10 | ✅ (`integration_test/scheduled_payments_patrol_test.dart`, 10 escenarios) | corrida `patrol-e2e-runner` 2026-09-10, worktree `feat/local-notifications`, emulator-5556, flavor `dev`, **REGRESIÓN sobre la corrida verde del 2026-09-09 (ver abajo)** | 🟡 **3/10 y 3/10 en 2 corridas independientes (run 2 y run 3), mismos 6-7 escenarios fallando en ambas — NO es flaky, es determinístico en este entorno.** Pasan consistentemente: `HU-01: crear`, `HU historial: recuperar un omitido`, `HU-05: eliminar`. Fallan consistentemente: `HU-05: editar` (`RenderFlex overflowed by 26px` en `scheduled_payment_form_page.dart:181`, **nuevo, no reportado en la corrida del 2026-09-09**), `HU confirmar antes de su fecha (81cb943)`, `bugfix scheduled-payment-skip-finish` y `HU calendar-fix-height-and-year-nav` (los 3 con `RenderFlex overflowed by 137-161px` en `lib/core/widgets/date_picker_sheet.dart:100`, **widget compartido, nuevo, no reportado antes**), `HU-08: configurar "3 días antes"` y `HU-08: cambiar a "Sin recordatorio"` (`RenderFlex overflowed by 21px` en `scheduled_payment_reminder_sheet.dart:68` — **esto es el mismo overflow que la corrida del 2026-09-09 vio "1 vez en ~6" y descartó como no reproducible; en esta corrida salió 2/2, es decir sí es un bug real, no flake**), y `HU-08: borrar con recordatorio` (no es overflow: `Expected: exactly one matching candidate / Actual: Found 0 widgets with text "Te avisamos una semana antes"`, línea 897 del test — puede ser bug de aserción del test o del widget, sin diagnosticar). Nada de esto se corrigió todavía — reportado tal cual a `flutter-dev`/`qa-automator` para diagnóstico. La entrada de abajo (2026-09-09) documenta la corrida previa que sí dio 10/10; algo entre esa corrida y esta volvió a romper 7 escenarios, o esa corrida no reprodujo lo que sí es reproducible aquí — a investigar antes de mergear el PR #29.

**Corrida previa (2026-09-09, contexto):** ✅ **10/10 en 2 corridas consecutivas** (5m45s / 6m9s). Partió de un 4/10 100% reproducible tras HU-08 (`ScheduledPaymentReminderField`): diagnóstico confirmó que los 6 fallos eran **bugs de test, no de `lib/`** — ningún overflow ni excepción de layout real. Causa raíz principal: `_scrollUntilVisible` arrastra en una sola dirección (`Offset(0, -250)`, siempre "hacia abajo"); varios escenarios ya visitaban campos/secciones fuera de orden top-to-bottom (p. ej. tocar "Manual" o escribir Nota — ambos *debajo* de "Primer pago" — antes de volver a elegir la fecha), y antes del campo de recordatorio el formulario cabía dentro del cache extent del `ListView` así que `dragUntilVisible` encontraba el elemento "gratis" sin arrastrar; con el formulario más alto, el campo de arriba queda fuera del árbol y el helper (que solo arrastra hacia abajo) nunca puede volver a alcanzarlo (`Bad state: No element`). Fix: `_scrollUntilVisible` ahora resetea al top (20 arrastres hacia abajo con el finger) antes de arrastrar hacia el target — direction-agnostic sin importar el orden en que cada escenario visite los campos. Segunda causa, en la página de detalle (también un `ListView` plano): varias aserciones (`'Omitido'`, `'Activa'`/`'Estado'`, y el segundo `'Internet'` de la fila de Historial) asumían visibilidad sin haber scrolleado, o intentaban verificar dos posiciones de scroll incompatibles en un solo `expect` (el hero arriba y la fila de Historial abajo no caben juntos en el cache extent una vez hay contenido real) — se separaron en aserciones por posición de scroll, con `'Historial'`/`'Estado'` como anclas de scroll (nunca `'Internet'`/`'Omitido'`, que ya están parcialmente presentes y no disparan el arrastre). Tercer hallazgo, un bug de aserción real en el propio test (no de scroll): el escenario `bugfix scheduled-payment-skip-finish` — cuyo **título propio** dice "recuperarlo lo vuelve Activa" — tenía el body comprobando `'Pendiente de confirmar'`; confirmado con un dump de todos los `Text` en pantalla que tras recuperar el estado real es "Activa" (la ocurrencia confirmada por "Confirmar ahora" se materializa en el `nextDate` futuro de la plantilla, y el path de lectura sin `force` de `_ensureDuePendingOccurrence` no la vuelve a mostrar como pendiente hasta que esa fecha llegue) — fix: la aserción ahora pide `'Activa'`, alineada con el título del escenario. **Hallazgo reportado, no corregido (posible bug real de `lib/`):** un `RenderFlex overflowed by 21 pixels` intermitente en `scheduled_payment_reminder_sheet.dart:68` (la hoja de 5 opciones de recordatorio) — su propio doc comment afirma que mide exacto en un viewport de 390x844 y por eso deliberadamente no usa `SingleChildScrollView`; el overflow observado sugiere que esa medición ya no es exacta en el emulator real (font scale/densidad, o una carrera de la animación de entrada) — visto 1 vez en ~6 corridas, no reproducido de forma determinística; reportar a `flutter-dev` para confirmar si es real antes de tocar el widget. Un fallo adicional visto una sola vez (`RenderFlex overflowed by 13px` en `scheduled_payment_form_page.dart:181`, la zona fija del monto) tampoco reprodujo en corridas subsecuentes — descartado como flake de esta máquina (ver `docs/dev-runs/` sobre flakiness conocida), no bug confirmado. Cobertura: HU-01 (crear), HU-05 (editar), "Confirmar ahora" (fix `81cb943`), historial + "Recuperar" (`6d15e61`), el bugfix skip-finish de `once`, HU-05 eliminar, el fix de calendario con vista de año, y las 3 nuevas de HU-08 (recordatorios: configurar "3 días antes", cambiar a "Sin recordatorio" al editar, borrar cancela la notificación pendiente). |
