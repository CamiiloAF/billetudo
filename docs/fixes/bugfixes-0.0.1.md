...
| 7 | ↩️ REVERTIDO (2026-07-24) | ~~PP reemplaza Metas en el nav; Metas en "Más" + accesos rápidos. Tab "Pagos"/`calendar-clock`.~~ **Revertido:** tras el rediseño de Metas (feature de re-enganche: momentum/racha/celebración), se reanalizó la IA del bottom nav y **Metas recupera el slot 4**; Pagos programados vuelve a "Más" + conserva su chip de acceso rápido en Inicio. El item 7 original fue una instrucción rápida anterior a que Metas tuviera por qué merecer el slot. Pendiente de aplicar en `.pen` (Tab Bar `u3b5s9` slot 4 → Metas/`target`), código (`home_tab_bar.dart`, shell, `more_page.dart`, `quick_access_row.dart`) y tests del nav. Además: "Más" se rediseña como menú estilo Mercado Libre (header + listas agrupadas). Ver `04-inicio.md` § Nota de navegación (2026-07-24). |
