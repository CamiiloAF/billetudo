...
## ✅ Patrol e2e — RESUELTO (2026-07-23, emulator-5554: 17/17 objetivo)

La corrida en device del 2026-07-22 (9/17) surfaceó un bug real + varios finders frágiles. Todo atacado el 2026-07-23:

1. **[BUG DE APP — RESUELTO]** El abono desde la hoja no bajaba el saldo (los 3 flujos + HU-07 saldada + Ledger). **Causa raíz real:** el CTA de `debt_payment_sheet.dart` estaba DENTRO del `SingleChildScrollView`; con el toggle "Sí" (default, hoja más alta) + el teclado que levanta el héroe autofocus, el botón quedaba bajo el fold en un teléfono real → el tap de submit no aterrizaba → el abono nunca se escribía. **NO era reactividad ni timing** (por eso "enlazar movimiento", que navega fuera y vuelve, sí funcionaba, y el `pump(500ms)` no lo movió). Fix: CTA como footer fijo fuera del scroll; mismo patrón aplicado a `debt_update_balance_sheet.dart` (defecto latente); 2 tests de regresión con teclado simulado; 40 goldens intactos. Ambas hojas con héroe+teclado quedan blindadas.
2. **[FINDERS — RESUELTO, qa-automator]** (a) `'No afecta cuentas'` matcheaba 2 filas (la de apertura también lleva ese tag, correcto) → scope al `DebtLedgerRow` del abono. (b) interés-auto `'estimado'` scrolleaba en dirección equivocada → sin scroll. (c) interés-auto rateField `:686` — finder compuesto `.first` lanzaba "No element" → scroll al `find.byKey('rate-new')` plano. (d) deep-link editar cuota `:412` — bug de test: el subtítulo es `debtContext`="{name} · {direction}" en un solo `Text` → `find.textContaining`.
3. **[NO ES BUG — decisión de producto]** Chip "Deuda" en PP (`:300`): una cuota **Manual** cae de inmediato en la sección "Por confirmar", que (por page-spec) NO renderiza `ScheduledCard` ni sus chips. El chip solo vive en la tarjeta activa. La app es correcta; el test asumía tarjeta activa. **Resolución elegida (Opción 1):** el escenario configura la cuota con fecha de primer pago futura → aparece como tarjeta activa con el chip. Sin cambio de app.

### Mejora de UX futura (anotada, NO bloqueante)
La sección **"Por confirmar"** (pending card/row) no muestra ninguna identidad de deuda: una cuota Manual —que vive ahí hasta que se confirma su ocurrencia— no se identifica como cuota de deuda en Pagos Programados hasta confirmarla. Evaluar si conviene añadir la identidad/chip "Deuda" al `ScheduledPendingCard`/`ScheduledPendingRow` (requiere frame en `billetudo.pen` + implementación + goldens). Prioridad baja: el detalle de la deuda ya muestra el cross-link completo y confirmar la ocurrencia hace lo correcto.
