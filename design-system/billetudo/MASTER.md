...
| `hint-text` | `#5648C8` | `#E4DEFA` | Cuerpo de las tiras informativas (`$primary-soft`) en los sheets de ajuste de monto. En oscuro es un lila casi-blanco (no el violeta de marca): antes el cuerpo usaba `primary-on-soft-strong`, que en oscuro es `#A78BFA` sobre `$primary-soft` `#26243B` — lila-sobre-lila apagado. `hint-text` sube a ~11:1 en oscuro; en claro es idéntico a `primary-deep`. |
...
**Mismo patron aplicado fuera de swatches (2026-08-27, corregido 2026-08-27):** el `Status Badge` de `Account Avatar` (`fPMzQ`, Home/Inicio) usa `fill:$amber` para el estado "requiere atencion" (respaldo con error/desactualizado), con un icono `cloud-off` encima — es el mismo caso de `$amber` aclarandose en oscuro sobre un fondo solido, **pero a diferencia de los swatches, aqui el icono NO se fija igual en ambos temas**: en **claro**, `$amber` es `#9B7608` (dorado-marron medio) y el blanco (`$on-primary`) ya pasaba bien (~4.21:1) — ese tema nunca fue el problema. En **oscuro**, `$amber` se aclara a `#FBDE24` (amarillo brillante) y blanco cae a ~1.36:1, ahi si hace falta el icono oscuro. La correccion es entonces un **override especifico de tema oscuro**, no un valor fijo para ambos: el componente base (`JOSLs`) mantiene `$on-primary` como default (funciona en claro, ~4.21:1), y las instancias en frames de tema oscuro que muestran el estado ambar (`SI6C4`, `gve6u`, `A4xpO`, `WfGHE`) sobreescriben ese icono a `#1C1B29` fijo (~12.58:1) via `descendants`. Los otros 3 estados del badge (sincronizado/sincronizando/sin cuenta) ya traen su propio override de icono/fill y no se tocaron. **Lección:** un fix de contraste medido en un solo tema (el que falló) no debe aplicarse ciegamente al otro tema sin volver a medir ahí — el primer intento de este mismo fix aplicó el valor oscuro a los dos temas por error, y aunque técnicamente pasaba el piso de 3:1 en claro (~4.03:1), se veía "apagado" a ojo — el usuario lo notó en el render antes de que ningún chequeo automático lo marcara.

**`on-primary-warn` (`#FFD27A`) y `on-primary-alert` (`#FFCFC4`) no tienen variante de tema — es intencional, no una omision.** Igual que `on-primary`, son valores fijos pensados para caer siempre sobre el gradiente `$primary`→`$primary-deep` del Hero (que no cambia de forma relevante entre temas), nunca sobre una superficie plana que si invierte por tema. Por eso el mismo hex pasa contraste en ambos modos. Uso confirmado en `Hero Compact · Presupuesto (D)` (`xRSdl`: `State Icon`, `Bar Projected`, `Risk Icon`). No recalibrar por tema sin verificar antes si el caso de uso sigue siendo "sobre el gradiente del hero".
...
- **Left Group + spacer invisible (2 acciones a la derecha):** cuando el lado derecho de un `Page Header` lleva 2 acciones (ej. `⋮` + `+`), no basta el spacer de 44x44 documentado arriba para "un lado sin accion" — ahi el lado izquierdo necesita un `Left Group` (`Back Button` + un `Spacer` invisible, `enabled:true`/`fill:[]`, del mismo ancho TOTAL que el grupo derecho: 44+8(gap)+44=96px) para que el titulo quede centrado matematicamente entre ambos grupos. Sin el spacer de grupo completo, el titulo se desplaza (~26px medido). Caso real: `REKRV` (Deudas) — `Tb4Ns` (`Left Group`) contiene `Back Button` + `qRvf4` (`Spacer` 44x44); el lado derecho (`Right Group`) lleva `Menu Button` + `Action Button`. Mismo patron aplica a `c196T3` (Pagos Programados), que en cambio solo tiene UNA accion a la derecha (el `Menu Button` reemplaza al `Action Button` por completo) y por eso no necesita `Left Group` — sigue el spacer simple de 44x44 ya documentado.
...
- **Voz: tuteo, siempre.** Nunca voseo ("Elige una pregunta", no "Elegí una pregunta"; "Inténtalo de nuevo", no "Intentá de nuevo"). Aplica a todo el producto — UI, copy de marketing, mensajes de error — sin excepcion por feature. Razon: el voseo es regional (Colombia paisa/valluna, Rio de la Plata, Centroamerica) y excluiria a Mexico, España, Peru, Chile y el Caribe; el tuteo es la unica forma que funciona en todo el mercado hispanohablante al que apunta el producto, y ademas es la mas calida de las neutras (el ustedeo suena distante). Decidido el 2026-08-26, tras que la feature de IA introdujera voseo y quedara inconsistente con el resto de la app. **Al barrer voseo con regex, `\b` no delimita vocales acentuadas** (`é`, `á`) en JS ni en varios motores: un barrido de `\b(elegi|intenta)\b` no encuentra "Elegí" ni "Intentá". Buscar tambien sin `\b`.
