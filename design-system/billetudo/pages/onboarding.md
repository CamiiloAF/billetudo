...
- **Campo "Institución (opcional)" agregado al paso 2** (2026-07-30, nodo `R7eph` claro / `exbxc` oscuro): revierte la ocultación de este campo específico frente al formulario completo de Cuentas — número de cuenta, tasa de interés, ícono y color siguen ocultos en el paso reducido, pero Institución se muestra porque es información que el usuario suele tener a mano al crear la cuenta. Reutiliza el mismo componente `Form Field` (`wOlOA`) y estilo que el campo equivalente de `pages/cuentas.md` (label, ícono `landmark`, placeholder "Opcional"). Se oculta cuando el tipo de cuenta es Efectivo (no tiene institución) — sin variante de mockup dedicada para ese estado oculto, mismo criterio que ya aplica `showInterestRateField` en Cuentas.
