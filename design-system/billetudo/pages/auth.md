...
| Auth — Fusión de datos, estado de error (`merge_confirmation_page_failure`) | — | — | **Sin frame dedicado** (gap real, no cerrado). Implementado reusando la estructura y tokens del componente reusable `Error State` (`awFv1`, mismo patrón ya auditado en `deudas.md` — `d64hv`/`tVUoU`): icono neutral `wifi-off` 40px, título `titleMedium`/700 (el código ya documenta la equivalencia con el `Title` de `Error State`, 16/700), subtítulo `bodyMedium`/`$text-secondary`, CTA de ancho completo. Verificado estructuralmente contra el componente compartido, no contra una pantalla propia de este flujo — si se quiere cerrar del todo, hace falta que `pencil-designer` dibuje un frame específico de Auth para este estado. |
...
| Auth — Borrar cuenta, paso 3: confirmación final (HU-07), `DeleteAccountScope.cloudAndLocal` | `sqm4I` | `q43mHJ` | Decisión final, ambos temas — pantalla completa, cierre neutral. Es la variante que diseñó el frame; las dos de abajo comparten estructura pixel-a-pixel (mismo layout, ícono, CTA) y solo cambian el copy vía `AppLocalizations`, decisión de negocio posterior al diseño (ver `account_deleted_page.dart`) |
| Auth — Borrar cuenta, paso 3, `DeleteAccountScope.localOnlySignedOut` | `sqm4I` | `q43mHJ` | Mismo frame que arriba (estructura idéntica verificada por golden). Copy no habla de la nube — este dispositivo cerró sesión de una cuenta real sin que esta corrida la contactara, así que no puede afirmar que la cuenta en la nube fue eliminada |
| Auth — Borrar cuenta, paso 3, `DeleteAccountScope.localOnlyNeverSignedIn` | `sqm4I` | `q43mHJ` | Mismo frame que arriba. Copy no menciona la nube en absoluto — `everSignedIn` es una bandera local sin verificación de servidor, así que el dispositivo podría tener una cuenta real que esta corrida nunca contactó |
