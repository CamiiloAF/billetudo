...
- **"Descartar" existe, pero acotado.** Decisión #22 originalmente lo eliminó del producto por completo (irreversible sobre datos que solo viven en el teléfono — exactamente lo que se perdió en ese incidente). El 2026-08-22 el dueño de producto la revirtió, de forma informada, con dos condados que la vuelven a hacer segura:
  - **Solo aparece con `attempts >= 3`.** Antes de ese umbral no está visible ni accesible en ninguna superficie — no se ofrece la salida destructiva ante el primer fallo.
  - **Una sola superficie de acceso:** el tercer botón (estilo `Delete Link`, `u0THG`) en la hoja de detalle (`r1qQYc`), nunca desde la fila de la lista (`rxUil`/`VtiBc`) — se evaluó agregarlo ahí también pero se descartó para no abrir una segunda superficie de una acción destructiva.
  - **Confirma siempre con hoja** (`qZvmL`, reusa el patrón `PqTUt`+`XPjIZ`+`Ot4yI` de "Cuentas — Confirmar Eliminar"), con copy explícito de que la acción es irreversible y borra solo el dato local de esa operación puntual — nunca todos los datos del teléfono.
- **"Descartar todo" (bulk), aprobado el 2026-08-25 — acción adicional, no reemplaza al descarte individual.** Nace de un caso real: 503 cambios en cuarentena por la colisión de ids entre cuentas (ver `docs/dev-runs/backup-restore-id-collision-remap.md`), donde descartar de a uno era inviable.
  - **Entrada:** link "Descartar todo (N)" (`OgoAn`) bajo el resumen de `rxUil` ("Ver los 89 pendientes"), mismo peso visual (`$expense-text`, peso 700) que el resto de acciones secundarias de esa familia — no un ícono en el header (variante descartada y borrada del canvas).
  - **Sin umbral.** A diferencia del individual (`attempts >= 3`), esta acción está siempre visible mientras haya al menos un pendiente — decisión explícita del dueño de producto, protegida solo por la hoja de confirmación.
  - **Confirma con el mismo patrón `qZvmL`** (nodeId: `EtwDY`). Título y cuerpo mantienen el conteo explícito ("¿Descartar los N cambios pendientes?" / irreversible, borra solo el dato local de esas operaciones específicas — nunca todo el teléfono), pero el **botón dice "Descartar todo"**, sin el número — a diferencia del botón individual, que sí lo lleva.
  - Usecase de dominio ya existente y sin usar hasta ahora: `DiscardAllQuarantinedOperations` (`SyncQuarantineRepository.clearAll()`).
...
- **No diseñado:** la animación de giro del `loader-circle`, el gesto de scroll de `rxUil`, y el comportamiento de la fila/contador tras confirmar un "Descartar" (individual o "Descartar todo") — animación de salida, snackbar de resultado, comportamiento si falla el borrado local de alguna operación durante el bulk.
