...
- **`AI Question Chip` (`tMqvn`), 2026-08-29 (v2, revierte stroke):** el chip (`fill:$muted`) se fundía con la card que lo contiene (`fill:$surface`) — 1.09:1/1.16:1, muy por debajo del 3:1 de WCAG 1.4.11. Un primer intento agregó `stroke:$primary-on-soft` 1.5px; el usuario lo rechazó al verlo en captura ("no me convence, se ve barato"). Se resolvió con COLOR: nuevo token `$muted-strong` (idéntico a `$muted` en claro, `#6E68AB` en oscuro, ~3.30:1 contra `$surface` oscuro), sin borde. Detalle completo en `MASTER.md` § AI Question Chip. Propaga automáticamente a toda instancia (`ref:tMqvn`) — el fill es del componente base, ninguna instancia lo sobreescribe.
- **Regla de altura dinámica por fila (issue #22, 2026-09-02):** el chip prefiere SIEMPRE 1 sola línea de texto (alto natural 44px). Cuando una pregunta real no cabe en 1 línea ni con el ancho generoso, necesita 2 líneas — y en ese caso **todos los chips visibles en esa fila/scroll comparten el mismo alto de 2 líneas**, aunque el texto de algunos quepa en 1 línea (nunca alturas mixtas dentro de la misma fila). Si ningún chip de la fila necesita 2 líneas, todos quedan en 44px. Referencia aprobada: `OshOq` ("Inicio · Chips IA — Variante A"), tarjeta `t1sfJ`, fila `XHTS0` ("Chips Scroll Row"). En Pencil esto sale gratis dejando `alignItems` sin fijar en la fila (default = stretch al hijo más alto); el `Chip 2` (angosto a propósito) envuelve a 2 líneas y los otros 3 se estiran automáticamente a esa misma altura — verificado: los 4 miden 64px de alto en este frame pese a que solo 1 lo necesita por contenido. Contrato completo para `flutter-dev` documentado en el `context` de `tMqvn`: la altura de fila se calcula en runtime según el chip más alto VISIBLE (típicamente el primero en cargar cada scroll de 4, recalculada si el usuario scrollea y aparece uno más alto) — implementar con `IntrinsicHeight`/stretch, nunca con un `height` fijo hardcodeado. **Divergencia actual con el código:** `ai_question_chip.dart` usa `$muted` crudo (no `$muted-strong`), fuerza 1 sola línea con ellipsis, y fuerza una altura fija de 64px entre los 4 chips de una fila sin importar si alguno la necesita — `ai_card_chips.dart` tampoco calcula el alto compartido dinámicamente. Las 3 divergencias las corrige `flutter-dev` en una sesión aparte, no se implementan aquí.
