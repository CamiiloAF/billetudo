...
3. **Chips Row** (scroll horizontal, `gap:8`) — **redefinida por completo desde el "Bottom sheet unificado de filtros" (GitHub issue #7, cierre 2026-09-02)**. Reemplaza toda la versión anterior de esta sección (un `Account Chip` dropdown con 3 estados + Fecha/Categoría/Tipo/Etiqueta como 4 chips más en la misma fila): Categoría/Tipo/Etiqueta viven ahora dentro del sheet unificado (se abre desde `Filtros Button` en la Search Row, no desde esta fila), y Cuenta pasó de un chip dropdown único a multi-select real, un chip por cuenta. Los 9 frames canónicos de esta fila (`B3GGa`/`xAk6Y` + sus 8 hermanos) quedaron desactualizados tras ese rediseño hasta corregirse el 2026-09-11 — ver "Cierre 2026-09-11" al final de esta sección.
   - **Chip "Todas"** (círculo 44×44, `cornerRadius:22`, ícono `check-check` 16px, fondo `$primary-soft` fijo, ícono `$text-secondary` — botón utilitario de "seleccionar todas", no un chip de estado activo/inactivo) — primero en la fila.
   ...
   - **Chip "Limpiar"** (mismo círculo que "Todas" pero ícono `x`, siempre en estado inactivo) — al final de la fila, vuelve el filtro a "todas seleccionadas".
   - **Chip Fecha condicional**: solo aparece, entre los `Account Chip` y "Limpiar", cuando el periodo activo NO es el default (mes calendario actual, sin presupuesto) — refleja el rango real ("Julio 2026" / "25 ago – 25 sep") en el mismo tratamiento de chip activo (mismo criterio que el Account Chip reflejando el nombre de cuenta). En el estado default no existe ningún chip de fecha en esta fila. Ver "Adición 2026-09-10 — Period Nav Bar" más abajo.
...
**Regla de interacción — 2 estados por tipo (documentada en HU-01/02/03 de `docs/requirements/fase-1/03-transacciones.md`, sección "Teclado numérico anclado"):**
...
- **Sheets sin Node ID propio en el `.pen` (golden sí existe, gap de documentación anotado 2026-08-04):**
  - **`sheet_date_range_picker_{light,dark}.png`**: mismo componente `Month Calendar`/`Date Picker Sheet` (`w4yuu`/`zMqxt`, ver "Selector de fecha del formulario" abajo) — no un componente distinto pese al nombre de archivo diferente al de `sheet_date_filter_custom_range` (que sí mapea a `OFdj4`/`mi0hB`). Visualmente idéntico, confirmado en la corrida de fidelidad 2026-08-04.
  - **`sheet_edit_impact_warning_scheduled_only_{light,dark}.png`**: segunda variante de copy del Aviso de Impacto Edición (`L9DJI`, vínculo específico a pago programado en vez de a meta) — misma estructura, sin frame propio para esta variante de texto.
  - **`sheet_future_date_scheduled_payment_prompt_{light,dark}.png`**: sheet "¿Es un pago programado?" del flujo de creación con fecha futura — cruza con la feature Pagos Programados (por eso vive en los goldens de Transacciones sin nodeId documentado aquí); ver `pagos-programados.md` si tiene su propia fila.
...
Solo quedan 2 huecos reales contra `docs/requirements/fase-1/03-transacciones.md` (todo el resto de pantallas/modales ya están construidos):
...
**Fidelidad visual — manija compartida (2026-08-04, `/design-fidelity-check transacciones`):** verificado con acceso real a Pencil. `movements_balance_carousel_collapsed_{light,dark}.png` es **fiel** al fix del 2026-08-03: la manija aparece como primer elemento de la franja superior, seguida de `d2TX3` sin su chevron final, en ambos temas. Cierra el pendiente anotado arriba. Sobre el título "Movimientos" descentrado: en esta pasada **ya no se observa** el hallazgo (se ve alineado a la izquierda en `transactions_page_with_data_*`) — parece corregido en algún punto posterior a 2026-07-21, sin registro explícito de cuándo.

**Chip Fecha en 2º lugar — cerrado, no era una divergencia real (2026-08-04):** el hallazgo IMPORTANTE original ("2º lugar en código vs. último en spec §3") comparaba contra el orden prosa de `docs/requirements/fase-1/03-transacciones.md` línea 63 ("cuenta(s), categoría(s), tipo, rango de fechas, etiqueta"), que **no es una spec de UI** — es solo una enumeración de HU-06, no ordenada a propósito. La sección "Chips Row" de este mismo documento (línea 82, arriba) ya registra la decisión **DEFINITIVA** del usuario del mismo 2026-07-21: `Cuenta → Fecha → Categoría → Tipo → Etiqueta`, con Fecha en 2º lugar a propósito. El código (`TransactionsFilterBar` en `transactions_page.dart`) implementa exactamente ese orden. Los dos hallazgos IMPORTANTE del carrusel quedan **ambos cerrados**: ninguno era una divergencia de código pendiente.
...
**Fidelidad visual (2026-08-04, `/design-fidelity-check transacciones`):** auditado con acceso real a Pencil. **Fiel** — verificado contra el código y contra la referencia `X3tZG` (Filter Account Row): icon-wrap **circular 44×44** (`colors.muted` de fondo), icono y **check en `colors.primaryOnSoft`** (fix de contraste WCAG aplicado correctamente en el golden actual, claro y oscuro), estructura `Bottom Sheet Base` + fila con rango de fechas + `Sheet Buttons Row` Limpiar/Aplicar, y estado vacío (icono `pie-chart` + mensaje) consistente con el resto de sheets de selección de la feature. Sin hallazgos CRÍTICO/IMPORTANTE. Cierra el pendiente anotado arriba.
