...
- **Tema oscuro**: los **tres selectores del formulario** (categoría `BobKK`/`fz53P`, cuenta `Zsrnf`, fecha `nYFOZ`) ya tienen copia oscura generada y auditada (zona `HXy6n`). El **resto** de las pantallas de la feature (Lista, Formularios completos, Detalle, Eliminar, Filtros) sigue sin copia oscura — se generan cuando esas queden 100% cerradas en claro (regla del proyecto: oscuro al final, después de componentizar lo repetido).
