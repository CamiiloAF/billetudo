...
**Organización del canvas — tema oscuro:** zona "TRANSACCIONES — OSCURO" (label `h6URn`, `x:50,y:23050`, sin banda decorativa, mismo criterio que el bloque claro), 24 pantallas en el mismo patrón de 4 filas (`y:23200`/`24350`/`25500`/`26650`), generadas con `Copy()` + `theme:{mode:"dark"}` desde cada frame claro — **recoloreo 100% automático, cero hex hardcodeado encontrado**. Verificado visualmente: contraste del Snackbar (`$snackbar-action` oscuro, 6.03:1), `primary-on-soft-strong` en categorías seleccionadas, `income-text` en monto de Ingreso, y el patrón de fila seleccionada en los filtros — todos legibles sin ajustes manuales.
...
- **Interacción real de mostrar/ocultar la Zona Fija** (animación de colapso, cómo se dispara el foco de "Nota" en Flutter): solo representada como 2 estados estáticos, falta especificar la transición.
- **Tema oscuro**: ninguna pantalla de esta feature tiene copia oscura todavía — se genera solo cuando **todo** lo de arriba esté cerrado en claro (regla del proyecto: oscuro al final, después de componentizar lo repetido).
