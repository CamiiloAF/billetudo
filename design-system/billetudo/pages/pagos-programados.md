...
| Detalle — híbrido (repetible activo) | `OY2Kj` (oscuro: `tlGsU`) |
...
- **Histórico → "Historial" con omitidos** (variante A aprobada; claro `PatSn`/`QbY9P`, oscuro `mar1S`): ya no es solo "generados". Es un **historial de eventos** de la plantilla, **intercalado cronológicamente** (más reciente arriba): pagos **confirmados** (transacción `source = scheduled`, fila `Transaction Row` `DKJaf`, enlaza a su transacción) y pagos **omitidos** (ocurrencia `skipped`, que **no** genera transacción). **3 filas + "Ver historial completo (N)"** in-place, mismo patrón/densidad que Presupuestos.
  - **"Ver más" reemplaza "Ver historial completo (N)"** (sincronizado 2026-07-29, ambos temas): el `TextButton` plano "Ver historial completo (N)" fue reemplazado por una instancia de `Load More · Ver más` (`oadHE`) en claro (`OY2Kj`) y oscuro (`tlGsU`, altura de frame corregida de 972→1098 para evitar clipping). Mismo look que Presupuestos/Deudas/Metas. Es un cambio puramente visual/de componente: el comportamiento de fondo (paginación real al servidor, de 8 en 8) no cambia. **Pendiente de decisión:** el componente `oadHE` no tiene variante visual de "cargando"; si se quiere un estado de carga explícito en el botón mientras trae más datos, falta diseñarlo — por ahora `flutter-dev` puede resolverlo con un estado inerte simple (opacidad reducida, sin interacción) sin bloquear la implementación.
