...
- **Stepper de periodo (HU-05) — pastilla flotante inferior:** NO es una fila arriba ni una barra de ancho completo (evita confundirse con la `Tab Bar`). Es una **píldora centrada flotante anclada abajo**, con el rango explícito del ciclo del propio presupuesto + estado: `‹ 21 jul – 20 ago · vigente ›`, NO un mes calendario global. (Este ejemplo decía `1–31 jul` y era justamente el error: un mes calendario. Estaba mal en el `.md` **y** en cuatro frames; se corrigió el 2026-07-19. No lo copies de vuelta.) El label va **partido en dos textos**, no en uno uniforme: rango 13/700 `$text-primary` + estado 12/600 `$text-secondary`. Los chevrons son **círculos `$muted` de 44pt**. En "una única vez" van **los dos** al 40%: hay un solo periodo, así que es índice 0 y último a la vez — no hay anterior ni siguiente. Siempre visible mientras la actividad scrollea. Controla qué periodo reflejan el hero y la actividad. Chevron deshabilitado en los bordes (no antes de `startDate` ni después de `endDate`). Alinear con el `DatePeriodFilter` de Transacciones.
  **Cierre 2026-09-11 — migra a componente compartido "Period Stepper" (deja de flotar):** `PeriodStepperPill` (`fU7hF`/`MZrD8`) queda reemplazado por el mismo componente reusable transversal **`Period Stepper`** (`vBgce`, ver `MASTER.md`) que ahora también usa Movimientos (`pages/transacciones.md` § "Period Nav Bar"), en su variante **V3 — pill inline**. Conserva la identidad visual (pill, chevrones circulares `$muted`, rango + estado en 2 fragmentos de texto) pero **cambia de comportamiento respecto al diseño anterior**: ya NO es flotante anclada al fondo de la pantalla — pasa a vivir en el flujo normal de contenido (inline).

  > **Pregunta abierta, sin resolver en el diseño — para `flutter-dev`/el usuario al implementar:** el diseño anterior garantizaba que el stepper estuviera "siempre visible mientras la actividad scrollea" (texto original de esta sección, arriba) precisamente por ser flotante. La variante inline aprobada NO resuelve esta garantía por sí sola — si se necesita conservar "siempre visible durante el scroll", requeriría implementarse como `sticky` en Flutter (viable, pero es una decisión de comportamiento nueva que el `.pen` no está especificando). Esta decisión queda explícitamente **pendiente**, no asumida a criterio de `pencil-designer`.
  >
  > **Alternativa sugerida por el usuario (2026-09-11), pendiente de construir:** en vez de `sticky` puro (fijo todo el tiempo) o inline puro (nunca flota), un comportamiento de **auto-hide por dirección de scroll**: el `PeriodStepper` aparece flotante cuando el usuario scrollea hacia arriba (intentando volver al inicio) y se oculta al scrollear hacia abajo — patrón común de barras de navegación contextuales (ej. Gmail/Chrome en Android). Aplicaría igual en Movimientos si se quiere consistencia entre ambas features. **Solo queda documentado por ahora — no implementado.** Antes de construirlo: decidir el mecanismo (`NotificationListener<ScrollNotification>` sobre la lista, comparando `scrollDelta`/dirección, con un `AnimatedSlide`/`AnimatedPositioned` para la transición), el umbral de sensibilidad (cuánto scroll dispara el cambio, para no parpadear con scrolls pequeños), y si aplica también a Movimientos o solo a Presupuestos.

  **Referencia visual aprobada:** `w3uTz` ("Presupuestos — Period Stepper (pill inline, casos 2 líneas)"), que muestra los 2 casos aplicables a Presupuestos — Budget Context Tag ("Comida del mes" + rango) y Rango+Estado ("25 ago – 25 sep · vigente"). Los frames canónicos `NloPT`/`vHIu4` (y el resto de detalles que instancian `fU7hF`/`MZrD8`) **no fueron actualizados todavía** en el `.pen` — siguen mostrando el pill flotante original. Queda pendiente una pasada de `pencil-designer` que reemplace `fU7hF`/`MZrD8` por una instancia de `vBgce` en los frames de detalle reales, y que decida junto al usuario/`flutter-dev` la pregunta de sticky-scroll de arriba antes de darlo por cerrado.
- **Actividad del periodo — expandir INLINE (no redirigir):** transacciones que cuentan para el presupuesto (reusa `Transaction Row`), excluye transferencias. El "ver más" **expande la lista in-place** con **"Ver más"** (paginación perezosa) — **nunca** redirige a una lista global (rompería el contexto de periodo + alcance). El header de la sección es `"Movimientos del periodo"` + el **contador** `"N movimientos"` a la derecha (`Nv04I`: `K7yvL` + `qFH6T`).
  - **Fila especial — Transferencia interna neteada** (`l3si5` claro / `HEgoB` oscuro, ambos temas aprobados 2026-08-17, frame de detalle "Transferencia Variante 2 (Fila neteada)"): cuando una transferencia presupuestable (`countsInBudget = true`, `03-transacciones.md`) tiene **origen y destino en el mismo presupuesto**, no se dibujan dos filas de gasto/ingreso enfrentadas — se muestra **una sola fila neteada** dentro de "Movimientos del periodo", instancia de `Transaction Row` (`DKJaf`) con overrides: icon-wrap `$primary-soft` + ícono **`arrow-left-right`** (no `repeat`/`repeat-1` — ese ícono ya lo usa el sheet "Ajustar monto" para una acción distinta; reusarlo aquí confundía visualmente "esto es una transferencia" con "esto es ajustar el monto del presupuesto") en `$primary-on-soft`, título **"Transferencia interna"**, subtítulo **"‹Cuenta A› ↔ ‹Cuenta B› · ‹fecha›"** y monto **`$0`** en `$text-secondary` — nunca verde (`income`) ni rojo (`expense`), porque no es un ingreso ni un gasto real, es un movimiento interno que se cancela a sí mismo dentro de ese presupuesto. Entre 2 variantes construidas (la otra: dos filas separadas gasto/ingreso), se descartó la de dos filas por inflar el conteo de movimientos y leerse como "gasté y luego gané" — contradice además el precedente ya aprobado de Transacciones (`xNp8g`), donde una transferencia se presenta como una sola entidad con dos cuentas visibles, no como dos movimientos.
    - **Regla de negocio y estado de implementación:** ver `docs/requirements/fase-1/06-presupuestos.md` línea ~148. El lado origen de una transferencia presupuestable ya contaba como gasto (`BudgetProgressCalculator`); el lado destino como ingreso por alcance de cuenta **y el neteo automático cuando ambas cuentas caen en el mismo presupuesto ya están implementados** (`BudgetsLocalDatasource`, `isIncome` reusando el mecanismo existente — cerrado 2026-08-17, cubierto en `test/features/budgets/data/budget_repository_impl_transfer_test.dart`). Esta fila es implementable de inmediato, no adelantada al dominio.
...
**Estado:** tema claro **aprobado** por el usuario (fusión final a los frames canónicos, 2026-08-07). Tema oscuro **pendiente** — se construye después.

**Patrón elegido:** acción de alternar dentro del sheet de acciones del detalle (`⋮`), no un toggle en la lista ni una fila aparte.

- **Sheet — acciones del detalle** (`JR1Xp` claro; `Z72OIH` oscuro aún sin actualizar), fila nueva entre "Editar" y "Ajustar monto — este período", ícono `star`, ya **fusionada al frame canónico** (ya no es un frame suelto):
  - **Estado OFF** (`kQSJH`, dentro de `JR1Xp`): label **"Usar como destacado en Inicio"** + subtítulo de exclusividad **"Reemplaza al presupuesto que tengas destacado, si hay uno."** (12/500 `$text-secondary`) — comunica antes de tocar que la acción es excluyente, sin sorpresas.
  - **Estado ON** (`n5eE9`, dentro del frame de referencia `R2vWv` — "Destacado ON, Claro"; el sheet real solo puede mostrar un estado a la vez, por eso el ON vive en un frame de documentación aparte en vez de fusionado en `JR1Xp`): label **"Quitar de Inicio"**, ícono `star-off`, sin subtítulo (ya es evidente qué hace). Usa `$primary-on-soft` (no `$primary` crudo) sobre `$surface`, según la excepción de contraste de `MASTER.md` para texto/ícono chico sobre superficie plana.
  - Icon-wrap y tipografía siguen el mismo patrón que el resto de filas del sheet (icon-wrap `$muted` 38/r12, título 15/600 `$text-primary`), tap target real 78px.

- **Señal visual en la lista** (componente `Budget Line` `FSL69`): badge circular de estrella ahora es un **nodo interno del componente reutilizable** (`WFz6L`, hijo `NFvw3` ícono `star`), no un overlay externo hardcodeado — 22×22px, fill `$primary`, stroke `$background` 2px, ícono `star` 12px `$on-primary`, `enabled:false` por defecto. Instanciado visible (`enabled:true`) en la card "Mercado del mes" (`brz28`) de la lista canónica `s833Gk`, como ejemplo del estado destacado. **Sin borde de card adicional** — el único diferenciador es el badge. El badge aparece igual sin importar si el destacado es manual o resultado del fallback automático (ver "Regla de negocio" abajo) — es un solo estado visual "destacado", sin distinguir origen.

**Regla de negocio:** solo puede haber **un** presupuesto destacado a la vez. Marcar uno nuevo desmarca automáticamente el anterior (no hay estado "varios destacados"). El subtítulo del estado OFF en el sheet es justamente el aviso de esa exclusividad antes de confirmar. Además del pick manual, existe un **fallback automático** (`BudgetHeroSelector._pickGlobalMonthly`): sin pick manual, el único presupuesto activo con alcance global + período mensual gana por defecto — el badge de la lista debe reflejar el resultado de ese selector, no solo el pick manual crudo (ver hallazgo de discoverability más abajo).

**Nodo IDs finales:**
| Pieza | Node ID |
|---|---|
| Sheet — acción "Usar como destacado" (estado OFF, fusionado en `JR1Xp`) | `kQSJH` |
