...
- **Gate de acceso a Pencil (obligatorio antes de implementar UI):** el `.pen` está encriptado — solo se lee con las herramientas MCP de Pencil, y **`flutter-dev` nunca las tiene** (por diseño, solo `pencil-designer`/`ui-ux-reviewer`/`pencil-fidelity-reviewer` las tienen). Por eso, antes de que `flutter-dev` toque la capa `presentation/` de cualquier pantalla diseñada, un agente **con acceso real** al `.pen` (`ui-ux-reviewer`) debe confirmar que puede verlo (editor state + captura de las pantallas de esa feature), no solo que existe el spec `.md`. **Si Pencil no es accesible, el desarrollo de esa UI se detiene** — no se implementa a ciegas contra el `.md` solo. Motivo: eso ya produjo deriva visual real (componentes Material genéricos en vez de los frames del `.pen`) en la corrida de Pagos Programados. El workflow `feature-dev` aplica este gate automáticamente antes de la fase Build cuando `needsUi = true`.
