...
Ya existe: esquema Drift (`lib/core/database/app_database.dart`, `schemaVersion` 9), base técnica cableada (`lib/core/`: DI, router, tema, l10n, errores, seguridad — ver "Cablear la base técnica de la app"), y varias features implementadas en `lib/features/`: `accounts`, `auth`, `budgets`, `categories`, `debts`, `goals`, `home`, `onboarding`, `reports`, `scheduled_payments`, `settings`, `transactions`, más `core/sync`. **Import/Export es la única pendiente real** — el router la sirve con `ComingSoonPage` de placeholder. `capture` e `improvement` existen como directorios vacíos, sin código todavía. No derives la estructura de una feature nueva por analogía a otro proyecto Flutter, sigue las convenciones de este documento.
